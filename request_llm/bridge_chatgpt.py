...
import requests
import importlib
# from crazy_functions.加群 import wepay_image_text, wechat_image_text
...
warning_list = [
    '1.请注意chatgpt的回复的知识并不是绝对正确的，可能出现一本正经的胡说八道的情况，请谨慎鉴别。',
    '2.为了防止泄露风险，请尽量不要将个人隐私和机密的核心信息输入chatgpt，以免造成信息泄露',
    '3.请遵守学术道德规范，不要将chatgpt生成的内容直接作为论文提交的版本',
    # '4.有兴趣的欢迎加入群讨论：'
]
warning_list2 = ['您的对话已经较长，为避免网络和服务器不稳定，建议使用右侧函数区黄色按钮保存对话记录，方便断连后加载']
            ...
            if chunk:
                try:
                    if len(json.loads(chunk.decode()[6:])['choices'][0]["delta"]) == 0:
                        # 怎么别人打开一次页面最多只弹出一次
                        if len(chatbot) == 2:
                            chatbot.append(('', 'TIPS:<br>' + '<br>'.join(warning_list)))
                            yield from update_ui(chatbot=chatbot, history=history, msg="回复完毕")  # 最后追加内容
                        elif len(chatbot) % 8 == 0:
                            chatbot.append(('', 'TIPS:<br>' + '<br>'.join(warning_list2)))
                            yield from update_ui(chatbot=chatbot, history=history, msg="回复完毕")  # 最后追加内容
                        else:
                            pass
